...
Varyings_Outline vertex_outline (Attributes_Outline input)
{
	...
	#elif TCP2_OBJECT_POS_AS_NORMALS
		// Object position gives low-poly/hard-edge meshes a continuous outline direction.
		float3 objectOutlineNormal = input.vertex.xyz;
		objectOutlineNormal = (dot(objectOutlineNormal, objectOutlineNormal) > 1.0e-8) ? objectOutlineNormal : input.normal;
		float3 normal = objectOutlineNormal;
	...
	/// #endif
		...
		float2 clipNormals = mul(UNITY_MATRIX_VP, float4(normal, 0)).xy;
		clipNormals /= max(length(clipNormals), 1.0e-5);
