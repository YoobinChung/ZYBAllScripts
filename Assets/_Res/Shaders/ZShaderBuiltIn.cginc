...
float2 ZShaderSelectUV(float uvSet, float2 uv1, float2 uv2, float2 uv3)
{
    if (uvSet > 1.5)
        return uv3;
...
ZShaderVaryings ZShaderBuiltInLitVertex(ZShaderAppData v)
{
    ...
    output.baseUV = TRANSFORM_TEX(ZShaderSelectUV(_BaseUV, v.uv, v.uv2, v.uv3), _BaseMap);
    output.normalUV = TRANSFORM_TEX(ZShaderSelectUV(_NormalUV, v.uv, v.uv2, v.uv3), _NormalMap);
    output.occlusionUV = TRANSFORM_TEX(ZShaderSelectUV(_OcclusionUV, v.uv, v.uv2, v.uv3), _OcclusionMap);
...
half3 ZShaderBuiltInGetShadingNormal(ZShaderVaryings input)
{
    ...
    {
        half3 normalTS = ZShaderUnpackNormalScale(tex2D(_NormalMap, input.normalUV), _NormalScale);
...
half4 ZShaderBuiltInLitFragment(ZShaderVaryings input) : SV_Target
{
    ...
    half4 baseSample = tex2D(_BaseMap, input.baseUV) * _BaseColor;
    ...
    {
        half aoSample = tex2D(_OcclusionMap, input.occlusionUV).r;
    ...
    {
        half3 emission = tex2D(_EmissionMap, input.baseUV).rgb * _EmissionColor.rgb;
...
ZShaderOutlineVaryings ZShaderBuiltInOutlineVertex(ZShaderAppData input)
{
    ...
    output.baseUV = TRANSFORM_TEX(ZShaderSelectUV(_BaseUV, input.uv, input.uv2, input.uv3), _BaseMap);
...
half4 ZShaderBuiltInOutlineFragment(ZShaderOutlineVaryings input) : SV_Target
{
    ...
    half alpha = tex2D(_BaseMap, input.baseUV).a * _BaseColor.a;
