Shader "CustomShader/ZShaderOutline"
{
    Properties
    {
        [MainTexture] _BaseMap ("Base Color", 2D) = "white" {}
        [MainColor] _BaseColor ("Base Tint", Color) = (1, 1, 1, 1)
        [Enum(UV1, 0, UV2, 1, UV3, 2)] _BaseUV ("Base UV", Float) = 0
        _AlbedoHSV_H ("Albedo Hue", Range(-180, 180)) = 0
        _AlbedoHSV_S ("Albedo Saturation", Range(-1, 1)) = 0
        _AlbedoHSV_V ("Albedo Value", Range(-1, 1)) = 0

        [Normal] _NormalMap ("Normal", 2D) = "bump" {}
        [Enum(UV1, 0, UV2, 1, UV3, 2)] _NormalUV ("Normal UV", Float) = 0
        ...
        [Enum(UV1, 0, UV2, 1, UV3, 2)] _OcclusionUV ("AO UV", Float) = 0
        ...
        _OutlineColor ("Outline Color", Color) = (0.04, 0.04, 0.04, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings
            {
                ...
                float2 occlusionUV : TEXCOORD5;
                UNITY_VERTEX_INPUT_INSTANCE_ID
                UNITY_VERTEX_OUTPUT_STEREO
            };

            float2 SelectUV(float uvSet, float2 uv1, float2 uv2, float2 uv3)
            {
                if (uvSet > 1.5)
                    return uv3;
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                output.baseUV = TRANSFORM_TEX(SelectUV(_BaseUV, input.uv, input.uv2, input.uv3), _BaseMap);
                output.normalUV = TRANSFORM_TEX(SelectUV(_NormalUV, input.uv, input.uv2, input.uv3), _NormalMap);
                output.occlusionUV = TRANSFORM_TEX(SelectUV(_OcclusionUV, input.uv, input.uv2, input.uv3), _OcclusionMap);
            ...
            half3 GetShadingNormal(Varyings input)
            {
                ...
                {
                    half3 normalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_NormalMap, sampler_NormalMap, input.normalUV), _NormalScale);
            ...
            half3 ApplyAlbedoHSV(half3 color)
            {
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                baseSample.rgb = ApplyAlbedoHSV(baseSample.rgb);
                ...
                {
                    half aoSample = SAMPLE_TEXTURE2D(_OcclusionMap, sampler_OcclusionMap, input.occlusionUV).r;
                ...
                {
                    half3 emission = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, input.baseUV).rgb * _EmissionColor.rgb;
        ...
        Pass
        {
            ...
            Varyings OutlinePassVertex(Attributes input)
            {
                ...
                float3 outlineDirectionOS = input.positionOS.xyz / max(length(input.positionOS.xyz), 0.0001);
                float3 positionOS = input.positionOS.xyz + outlineDirectionOS * _OutlineWidth;
                output.positionCS = TransformObjectToHClip(positionOS);
                output.uv = TRANSFORM_TEX(SelectUV(_BaseUV, input.uv, input.uv2, input.uv3), _BaseMap);
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CGPROGRAM
            #pragma target 3.0
            #pragma vertex ZShaderBuiltInLitVertex
            #pragma fragment ZShaderBuiltInLitFragment
            #pragma multi_compile_instancing
            #pragma multi_compile_fwdbase

            #include "ZShaderBuiltIn.cginc"
            ENDCG
        }
        ...
        Pass
        {
            ...
            CGPROGRAM
            #pragma target 3.0
            #pragma vertex ZShaderBuiltInOutlineVertex
            #pragma fragment ZShaderBuiltInOutlineFragment
            #pragma multi_compile_instancing
        ...
        Pass
        {
            ...
            CGPROGRAM
            #pragma target 3.0
            #pragma vertex ZShaderBuiltInShadowVertex
            #pragma fragment ZShaderBuiltInShadowFragment
            #pragma multi_compile_instancing
