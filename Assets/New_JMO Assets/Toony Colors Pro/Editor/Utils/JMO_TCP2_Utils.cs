...
namespace JMOToonyColorsPro
{
	namespace Utilities
	{
		public static class JMO_TCP2_Utils
		{
			//--------------------------------------------------------------------------------------------------------------------------------
			...
			public static bool ShaderKeywordToggle(string keyword, string label, string tooltip, List<string> list, ref bool update, string helpTopic = null)
			{
				...
				if (!string.IsNullOrEmpty(helpTopic))
				{
					...
					JMO_TCP2_GUI.HelpButton(helpTopic);
				}
			...
			public static bool ShaderKeywordRadio(string header, string[] keywords, GUIContent[] labels, List<string> list, ref bool update)
			{
				...
				if (hasHeader)
				{
					...
					JMO_TCP2_GUI.HelpButton(helpRect, helpTopic);
			...
			public static string FindReadmePath(bool relativeToAssets = false)
			{
				...
				if (readmePathFull == null)
				{
					...
					string readmePath = GetFileSafe(Application.dataPath, "!JMOToonyColorsPro Readme.txt");
				...
				if (readmePathFull == null)
				{
					Debug.LogError("Couldn't find the Toony Colors Pro root directory. Make sure the Editor/Utils, Shader Generator, or Shader Templates 2 folders are imported.");
					return null;
				}
			...
			{
				string rootPath = FindRootPathFromAssetSearch("JMO_TCP2_Utils t:MonoScript", "/Editor/Utils/JMO_TCP2_Utils.cs");
