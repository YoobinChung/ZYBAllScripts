...
using UnityEditor;
using UnityEngine;
using JMOToonyColorsPro.Utilities;
using System.IO;
...
namespace JMOToonyColorsPro
{
	...
	{
		public class JMO_MaterialInspector_Hybrid : ShaderGUI
		{
			...
			public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
			{
				...
				// Header
				GUILayout.Label(JMO_TCP2_GUI.TempContent(EditorGUIUtility.currentViewWidth > 355f ? "JMO Toony Colors Pro 2 - Hybrid Shader" : "JMO TCP2 - Hybrid Shader"), OrangeHeaderStyle);
				JMO_TCP2_GUI.Separator();
				...
				// Specific
				GUILayout.Label(JMO_TCP2_GUI.TempContent("Transparency"), OrangeBoldLabelStyle);
				HandleRenderingMode();
			...
			void HandleOutlinePass(MaterialProperty outlineProp)
			{
				...
				EditorGUI.showMixedValue = outlineProp.hasMixedValue;
				{
					...
					_materialEditor.ShaderProperty(outlineProp, JMO_TCP2_GUI.TempContent(outlineProp.displayName));
			...
			void DisplayOutlineWarning(MaterialProperty outlineProp)
			{
				...
				if (outlineProp.floatValue > 0 || outlineProp.hasMixedValue)
				{
					...
					if (srp != null && srp.GetType().ToString().Contains("Universal"))
					{
						...
						{
							GUILayout.Label(JMO_TCP2_GUI.TempContent("Universal Render Pipeline requires some additional setup for the Outline to work, using the 'Renderer Features' system."), EditorStyles.wordWrappedLabel);
							GUILayout.BeginHorizontal();
							{
								GUILayout.FlexibleSpace();
								if (GUILayout.Button(JMO_TCP2_GUI.TempContent("See documentation"), GUILayout.ExpandWidth(false)))
								{
			...
			void HandleMobileMode()
			{
				...
				{
					var newMobileMode = (MobileMode)EditorGUILayout.EnumPopup(JMO_TCP2_GUI.TempContent("Mobile Mode", mobileModeHelp), (MobileMode)mobileModeProp.floatValue);
			...
			void HandleRenderingMode()
			{
				...
				{
					var newRenderingMode = (RenderingMode)EditorGUILayout.EnumPopup(JMO_TCP2_GUI.TempContent("Rendering Mode"), (RenderingMode)renderingModeProp.floatValue);
		...
		internal class GC_Separator : GUICommand { public override void OnGUI() { if (JMO_MaterialInspector_Hybrid.ShowNextProperty) JMO_TCP2_GUI.SeparatorSimple(); } }
		internal class GC_SeparatorDouble : GUICommand { public override void OnGUI() { if (JMO_MaterialInspector_Hybrid.ShowNextProperty) JMO_TCP2_GUI.Separator(); } }
		internal class GC_Space : GUICommand { public override void OnGUI() { if (JMO_MaterialInspector_Hybrid.ShowNextProperty) GUILayout.Space(8); } }
		internal class GC_HelpBox : GUICommand
		{
			...
			{
				if (JMO_MaterialInspector_Hybrid.ShowNextProperty)
					JMO_TCP2_GUI.HelpBoxLayout(message, messageType);
			}
		}
		internal class GC_Header : GUICommand
		{
			...
			public override void OnGUI()
			{
				...
				if (JMO_MaterialInspector_Hybrid.ShowNextProperty)
					GUILayout.Label(guiContent, JMO_MaterialInspector_Hybrid.OrangeBoldLabelStyle);
			}
		}
		...
		internal class GC_IfKeyword : GUICommand
		{
			...
			public override void OnGUI()
			{
				bool show = JMO_TCP2_ExpressionParser.EvaluateExpression(expression, (string s) =>
				{
				...
				JMO_MaterialInspector_Hybrid.PushShowProperty(show);
			}
		}
		internal class GC_IfURP : GUICommand
		{
			...
			public override void OnGUI()
			{
				bool show = JMO_MaterialInspector_Hybrid._isURP;
		...
		internal class GC_Else : GUICommand
		{
			public override void OnGUI()
			{
				bool invertCondition = !JMO_MaterialInspector_Hybrid.ShowNextProperty;
				JMO_MaterialInspector_Hybrid.PopShowProperty();
				JMO_MaterialInspector_Hybrid.PushShowProperty(invertCondition);
			}
		}
		internal class GC_EndIf : GUICommand { public override void OnGUI() { JMO_MaterialInspector_Hybrid.PopShowProperty(); } }
		internal class GC_EndIfDisable : GUICommand { public override void OnGUI() { JMO_MaterialInspector_Hybrid.PopDisableProperty(); } }
		...
		internal class GC_IfProperty : GUICommand
		{
			...
			public override void OnGUI()
			{
				bool show = JMO_TCP2_ExpressionParser.EvaluateExpression(expression, EvaluatePropertyExpression);
				JMO_MaterialInspector_Hybrid.PushShowProperty(show);
			}
		...
		internal class GC_IfDisableProperty : GC_IfProperty
		{
			public override void OnGUI()
			{
				bool enable = JMO_TCP2_ExpressionParser.EvaluateExpression(expression, EvaluatePropertyExpression);
				JMO_MaterialInspector_Hybrid.PushDisableProperty(!enable);
			}
		}
		...
		internal class GC_IfDisableKeyword : GC_IfKeyword
		{
			public override void OnGUI()
			{
				bool enable = JMO_TCP2_ExpressionParser.EvaluateExpression(expression, (string s) =>
				{
					foreach (var m in materials)
					{
						if (m.IsKeywordEnabled(s))
